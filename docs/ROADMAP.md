...
- [x] **6. Composition root** — argparse `run`/`eval`; dotenv; per-doc isolation (any exception → `fallback_decision` → review); copy with sanitized LLM paths (whitelist chars incl. Greek, traversal impossible), collision suffixes `_2, _3…`, no double extensions; `output/decisions.jsonl` (machine record for eval) + review CSV; per-doc console line + end summary; `--model` flag; mocked end-to-end test incl. hostile-filename case. *Review fixed (adversarial probing): component length capped at 120 chars + copy failures demoted to review instead of killing the run (reviewer demonstrated a prompt-injected 300-char folder name aborting the whole run via WinError 123); pipeline errors now visible on console; suffix-less-source edge in `target_filename`; COM0/LPT0/superscript reserved names; CLI tests added. Containment itself held under ~35 hostile path probes.*
